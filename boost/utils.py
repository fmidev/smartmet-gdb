...
class GDB_Value_Wrapper(gdb.Value):
    ...
    def __init__(self, value):
        ...
        if have_python_2:
            self.__dict__ = {}
        # FMI local patch: upstream calls `gdb.Value.__init__(value)` which is
        # missing `self` -- on gdb >= ~13 / Python 3.11+ gdb.Value.__init__
        # enforces its `val` argument, so every printed value raised
        # "TypeError: function missing required argument 'val'". See
        # UPSTREAM_VERSION.txt. Reapply if you re-vendor from upstream.
